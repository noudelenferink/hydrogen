...
import { TrainingManagerPage } from '../training-manager/training-manager';
...
export class TrainingsManagerListPage {
  ...
  loadTraining(event, training) {
    this.app.getRootNav().push(TrainingManagerPage, {
  ...
  promptDelete(training) {
    var self = this;
    var formattedDate = new Date(training.TrainingDate).toLocaleDateString();
    let confirm = this.alertCtrl.create({
      title: 'Training verwijderen',
      message: `Weet je zeker dat je de geselecteerde training van <b>${formattedDate}</b> wilt verwijderen?`,
      buttons: [
        {
          text: 'Annuleren',
          handler: () => { }
